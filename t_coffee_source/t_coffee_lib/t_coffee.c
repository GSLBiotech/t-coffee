...
int batch_main ( int argc, char **argv)
{
  ...
  */
  ...
  if (t_coffee_defaults_flag)
  {
    ...
    {
      declare_name(pname);sprintf (pname, "%s/.t_coffee_defaults",getenv(ENV_HOME) );
  ...
  if (CL->ne>0 && out_lib[0]!='\0' && !strm (out_lib, "no"))
  {
    if (strstr (out_lib_mode, "extended"))
    {
      ...
      pOUT=vfopen (out_lib, "w");
      pOUT=save_extended_constraint_list(CL,emode,pOUT);
    }
    else
    {
      pOUT=save_constraint_list ( CL, 0, CL->ne, out_lib, NULL, "ascii",CL->S);
    }
    vfclose (pOUT);
  ...
  else if ( do_genepred)
  {
    ...
    for ( b=0; b< n_out_aln_format; b++)
    {
      ...
      pOUT=copy_aln (A2,NULL);
      output_format_aln (out_aln_format[b],pOUT,NULL, tot_out_aln[b]);
